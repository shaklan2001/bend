...
const DurationModal = memo(({ visible, onClose, exercises, onSave }: DurationModalProps) => {
  ...
  const handleDurationChange = useCallback((exerciseId: string, newDuration: number) => {
    setExercisesWithDuration(prev =>
      prev.map(exercise =>
        exercise.id === exerciseId ? { ...exercise, duration_seconds: newDuration } : exercise
      )
    );
  }, []);
  ...
  const handleDesignCoverFinish = useCallback(
    async (routineData: {
      name: string;
      exercises: ExerciseWithDuration[];
      coverImage: string;
      ...
      try {
        const totalDuration = routineData.exercises.reduce(
