...
const ExerciseInfo = memo(
  ({ exercise, onInfoPress }: { exercise: Exercise; onInfoPress: () => void }) => {
    return (
      <View className='items-center px-6 mb-5'>
        <View className='flex-row items-center'>
          <Text style={exerciseInfoStyles.exerciseName}>{exercise.name}</Text>
          <TouchableOpacity
...
export const ExerciseModal = memo(
  ({ visible, exercises, onClose, onComplete, routineName, routineSlug }: ExerciseModalProps) => {
    ...
    const totalMinutes = useMemo(() => {
      return Math.round(
        exercises.reduce((total, exercise) => total + exercise.duration_seconds, 0) / 60
      );
    }, [exercises]);
          ...
          <View className='flex-1 bg-white'>
            ...
            <View className='flex-1 mt-2.5'>
              <View className='relative items-center justify-center'>
                ...
                <TimerCircle progress={progress} size={screenWidth * 0.87} />
              </View>
              ...
              <ExerciseInfo exercise={currentExercise.exercise} onInfoPress={handleInfoPress} />
