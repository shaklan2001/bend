...
const CoverImage = memo(({ imageUrl, size = 50 }: { imageUrl: string; size?: number }) => {
  ...
  return <Image source={{ uri: imageUrl }} style={imageStyle} resizeMode='cover' />;
});
...
const CoverPreviewCard = memo(
  ({ exercises, routineName, onPress, showTouchable = false }: CoverPreviewCardProps) => {
    ...
    if (showTouchable && onPress) {
      return (
        <TouchableOpacity onPress={handlePress} activeOpacity={0.8} className='mx-8 mb-4'>
          {cardContent}
        </TouchableOpacity>
      );
    }

    return <View style={{ marginBottom: 32 }}>{cardContent}</View>;
  }
);
