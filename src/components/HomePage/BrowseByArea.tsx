...
const AreaCardComponent = memo(
  ({ area, onPress }: { area: AreaCard; onPress: (area: AreaCard) => void }) => (
    <TouchableOpacity
    ...
    >
      ...
      <Text className='text-m font-bold text-black -mt-2 text-center'>{area.name}</Text>
    </TouchableOpacity>
  )
);

const SectionHeader = memo(({ title }: { title: string }) => (
  <Text className='text-base font-bold text-[#A69B8A] mb-5 uppercase tracking-wider'>{title}</Text>
));

const LoadingState = memo(() => (
  <View className='px-6 py-6'>
    <SectionHeader title='BROWSE BY AREA' />
    <Text className='text-base text-gray-400 text-center'>Loading areas...</Text>
  </View>
));
...
const EmptyState = memo(() => (
  <View className='px-6 py-6'>
    <SectionHeader title='BROWSE BY AREA' />
    <Text className='text-base text-gray-400 text-center'>No areas available</Text>
  </View>
));

const AreaCardsRow = memo(
  ({ areas, onAreaPress }: { areas: AreaCard[]; onAreaPress: (area: AreaCard) => void }) => (
    <View className='flex-row mb-4'>
      {areas.map(area => (
        <AreaCardComponent key={area.id} area={area} onPress={onAreaPress} />
      ))}
    </View>
  )
);
...
const AreaCardsContainer = memo(
  ({
    ...
    >
      <View>
        <AreaCardsRow areas={bodyParts.slice(0, 3)} onAreaPress={onAreaPress} />
        <AreaCardsRow areas={bodyParts.slice(3, 6)} onAreaPress={onAreaPress} />
      </View>
    </ScrollView>
  )
);
...
const BrowseByArea = memo(
  ({ bodyParts = [], isLoading = false, error = null, onRetry }: BrowseByAreaProps) => {
    ...
    const hasBodyParts = useMemo(() => bodyParts && bodyParts.length > 0, [bodyParts]);
    ...
    return (
      <>
        <View className='px-6 py-6'>
          <SectionHeader title='BROWSE BY AREA' />
          <AreaCardsContainer bodyParts={bodyParts} onAreaPress={handleAreaPress} />
        </View>
        {isRoutineBottomSheetVisible && (
          <RoutineBottomSheet
