          ...
          >
            <Image
              source={
                data.image
                  ? data.image
                  : { uri: data.image_url || 'https://placehold.co/60x60/f3f4f6/9ca3af?text=Yoga' }
              }
              style={{
                width: 50,
                height: 50,
...
export const RecommendedCard = memo(
  ({ data, index, scrollX }: { data: RoutineCard; index: number; scrollX: Animated.Value }) => {
    return <BaseRecommendedCard data={data} index={index} scrollX={scrollX} showAnimation={true} />;
  }
);
    ...
    <View style={{ paddingVertical: 10 }}>
      ...
      >
        {recommendedData.map((item, index) => (
          <RecommendedCard key={index} data={item} index={index} scrollX={scrollX} />
        ))}
      </ScrollView>
    </View>
  );
