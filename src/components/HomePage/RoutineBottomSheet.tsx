      ...
      >
        ...
        >
          ...
          >
            <Image
              source={{
                uri: routine?.image_url || 'https://placehold.co/60x60/f3f4f6/9ca3af?text=Yoga',
              }}
        ...
        <View className='flex-1 ml-2'>
          <Text className='text-2xl font-bold text-gray-900 mb-1'>{routine.name}</Text>
...
const Header = memo(({ bodyPartName, onClose }: { bodyPartName: string; onClose: () => void }) => (
  <View className='flex-row items-center justify-between px-6 py-3 border-b border-gray-100'>
    ...
    <View className='flex-1 items-center'>
      <Text className='text-2xl font-bold text-gray-900'>{bodyPartName}</Text>
    </View>
...
const LoadingState = memo(() => (
  <View className='py-10 items-center'>
    <Text className='text-base text-gray-400'>Loading routines...</Text>
  </View>
));
...
const RoutineBottomSheet: React.FC<RoutineBottomSheetProps> = memo(
  ({ visible, onClose, bodyPart, routines, isLoading = false }) => {
